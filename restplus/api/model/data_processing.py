...
def getTokenizer(tokenizer=True, tokenizer_name='tokenizer.pickle', vocabSize=0, data=None):
	...
	"""
	path = os.path.join(os.getcwd(),'restplus/api/model/Encoder',tokenizer_name)
	if tokenizer == False:
...
def get_sklearn_onehot_econder(data, encoder_name, train=True):
	...
	"""
	path = os.path.join(os.getcwd(),'restplus/api/model/Encoder', encoder_name)
	if train:
...
def get_one_hot_encode_for_model(pd_df, train=True):
	...
	data = pd_df.drop(['Division Name', 'Department Name', 'Class Name'], axis=1)
